...
//    painter->fillRect(QRect(340, 115, 50, 50), QBrush(QColor(Qt::black)));
    ...
    if (value < alarmvalue) {
        ...
        painter->setBrush(QBrush(linearGradient_body));//设置填充形式
//        painter->fillRect(QRect(35, 45, 3 * value, 190),  QBrush(QColor(Qt::red)));
        ...
        linearGradient_body.setColorAt(1, Qt::black);//设置电池颜色填充色
        painter->setBrush(QBrush(linearGradient_body));//设置填充形式
//        painter->fillRect(QRect(35, 45, 3 * value, 190), QBrush(QColor(Qt::green)));
        painter->drawRoundedRect(QRect(35, 45, 3 * value, 190), back_angle, back_angle);//画电池填充色
    }
    painter->end();
}
